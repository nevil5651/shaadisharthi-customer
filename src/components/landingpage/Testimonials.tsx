            ...
            <div key={index} className="bg-white p-8 rounded-xl shadow-md relative">
              ...
              <div className="flex items-center mb-6">
                {testimonial.images.map((img, i) => (
                  <Image key={i} src={img} alt="Client" width={64} height={64} className={`w-16 h-16 rounded-full object-cover border-4 border-primary border-opacity-20 ${i > 0 ? '-ml-4' : 'mr-4'}`} />
                ))}
                <div className={testimonial.images.length > 1 ? 'ml-4' : ''}>
                  <h4 className="font-bold font-playfair-display">{testimonial.name}</h4>
