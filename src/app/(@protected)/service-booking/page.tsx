...
// Memoized loading component to prevent recreating on every render
const LoadingState = () => (
...
// Memoized service info component
const ServiceInfo = ({ service }: { service: NonNullable<ReturnType<typeof useServiceBooking>['service']> }) => (
...
// Memoized service header component
const ServiceHeader = ({ serviceName }: { serviceName: string }) => (
  <div className="bg-gradient-to-r text-center mb-4 from-primary/5 to-secondary/5 dark:from-primary/10 dark:to-secondary/10 p-6 border-b border-gray-100 dark:border-gray-700">
    <h1 className="text-3xl font-bold font-serif text-gray-800 dark:text-white">Book {serviceName}</h1>
...
const ServiceBookingForm = () => {
  ...
  // Combined effects for setting form values and document title
  useEffect(() => {
    if (service) {
      // Batch form value updates
